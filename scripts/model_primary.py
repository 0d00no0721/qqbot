...
# ---------- 配置（从 .env 读取默认值） ----------
# 这些是默认值，可以从 stdin 的 model/api_base_url/api_key 字段覆盖
import os
    ...
    try:
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            ...
            if resp.status_code != 200:
                import sys; print(f"[model_primary] API返回非200: {resp.status_code} {resp.text}", file=sys.stderr, flush=True)
                return ""
            data = resp.json()
        ...
        import sys; print(f"[model_primary] API调用失败: {e} | url={API_BASE_URL} model={API_MODEL}", file=sys.stderr, flush=True)
        return ""
